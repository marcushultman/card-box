...
export const handler: Handlers = {
  async POST(req, _) {
    ...
    const userid = (data.get('userid') as string | null)?.toLowerCase();
    const pwd = data.get('pwd');

    const useridToPwd = new Map(
      (Deno.env.get('ALPHA_USERS') ?? '').split(',').map((u) =>
        u.split(':', 2) as [string, string]
      ),
    );

    if (!userid || useridToPwd.get(userid) !== pwd) {
    ...
    setCookie(headers, { name: 'jwt', value: jwt });
...
export default function login() {
  return (
    <form class='p-4 mx-auto max-w-screen-md h-screen relative flex(& col)' method='post'>
      ...
      <label for='userid'>[BETA] UserID:</label>
      <input
        type='text'
        id='userid'
        name='userid'
